...
function page() {
  ...
  const fetchGenresMoviesAndDetails = async (endpoint: string) => {
    ...
    useEffect(()=>{
      ...
      let genresEndpoint: string = `${API_URI}genre/movie/list?api_key=${API_KEY}&language=ko-KR`; //장르 ID 정보
      let discoverMoviesEndpoint: string = `${API_URI}discover/movie?api_key=${API_KEY}&language=ko-KR&with_genres=${changeGenres}&sort_by=${change}.${changeDescAsc}`//&with_genres=${GENRE_ID}&sort_by=popularity.desc&page=1` //장르별 영화들
      ...
      fetchGenres(genresEndpoint);

      //fetchGenresMovies(endpointGenresMovies);
      //fetchRevenueAndBuget();
      //위 두개 합침
      fetchGenresMoviesAndDetails(discoverMoviesEndpoint);
      
    },[]); //genresMoviesInfo  revenueAndBudgetInfo
